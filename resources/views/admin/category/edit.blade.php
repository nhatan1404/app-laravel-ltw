      ...
      <div class="card-body">
        <form method="post" action="{{route('category.update',$category->id)}}">
          ...
          <div class="form-group">
            ...
            <textarea class="form-control" id="inputDescription" name="description">{{$category->description}}</textarea>
            @error('description')
            <span class="text-danger">{{$message}}</span>
            @enderror
          </div>
          ...
          <div class="form-group {{(($category->parent_id==null) ? 'd-none' : '')}}">
            ...
            <select name="parent_id" class="form-control">
              ...
              <option value='{{$parentCategory->id}}' {{(($parentCategory->id==$category->parent_id) ? 'selected' : '')}}>{{$parentCategory->title}}</option>
              @endforeach
            </select>
          </div>
